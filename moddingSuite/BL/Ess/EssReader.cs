...
public class EssReader
{
    ...
    public byte[] ReadEss(byte[] data)
    {
        ...
        _ = br.ReadInt32(); // skip ESS version
        ...
        _ = br.ReadInt32();
        _ = br.ReadInt32(); // num_samples again

        Writewavheader(bw, channels, samplerate, numSamples * channels * 2);

        int nfr = 1;
        // dumb end search
        while (inMs.Position + (int)Swap(br.ReadUInt32()) + 4 < inMs.Length)
            nfr++;
        int datapos = (int) inMs.Position;

        _ = inMs.Seek(0x14, SeekOrigin.Begin);
        ...
        for (int j = 0; j < nfr; j++)
            frs[j + 1] = (int)Swap(br.ReadUInt32());
        ...
        for (int j = 0; j < nfr; j++)
        {
            ...
            _ = inMs.Seek(datapos + frs[j] + (20 * channels), SeekOrigin.Begin);
            _ = inMs.Read(frame, 0, frSize);
            _ = inMs.Seek(datapos + frs[j], SeekOrigin.Begin);
